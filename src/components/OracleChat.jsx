...
- If someone asks something outside Batman lore, say: "That's outside my database. I run Batman intel, not a general encyclopedia. Try again."
...
export default function OracleChat() {
    ...
    const [messages, setMessages] = useState([
        { role: 'oracle', text: "Oracle online. Clocktower systems nominal. What do you need to know about Gotham?" },
    ]);
    ...
    const send = async () => {
        ...
        if (!apiKey) {
            setMessages(prev => [...prev, { role: 'oracle', text: "Connection error. VITE_GROQ_API_KEY not configured." }]);
            setLoading(false);
            return;
        }
        ...
        try {
            ...
            const reply = data?.choices?.[0]?.message?.content || "Signal lost. No data returned.";
            setMessages(prev => [...prev, { role: 'oracle', text: reply }]);
        } catch (err) {
            setMessages(prev => [...prev, { role: 'oracle', text: `Satellite uplink failed: ${err.message}` }]);
        }
            ...
            {open && (
                <div className="oracle-panel">
                    <div className="oracle-header">
                        ...
                        <button className="oracle-close" onClick={() => setOpen(false)} aria-label="Close">✕</button>
                    </div>
                    ...
                    <div className="oracle-messages">
                        ...
                        {loading && (
                            <div className="oracle-msg oracle-msg--oracle">
                                ...
                                <p className="oracle-msg-text oracle-typing"><span /><span /><span /></p>
                            </div>
                        )}
                        <div ref={bottomRef} />
                    </div>
                    ...
                    <div className="oracle-footer-note">Batman lore only · Powered by Groq</div>
                </div>
            )}
        </>
    );
